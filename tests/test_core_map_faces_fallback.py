...
def _patched_core_import_stubs(calls, pipe_result=False):
    ...
    stubs = {
        ...
        "modules.metadata": types.SimpleNamespace(name="Portrait Relay", version="test"),
        "modules.ui": types.SimpleNamespace(
            screen_explicit_content=lambda *_args, **_kwargs: False,
        ...
        "modules.processors.frame.core": types.SimpleNamespace(
            ...
            process_video_in_memory=lambda *_args, **_kwargs: (
                calls.append(("pipe",)) or pipe_result
            ),
        ),
        "modules.utilities": types.SimpleNamespace(
            ...
            create_video=lambda target_path, fps: (
                calls.append(("create_video", target_path, fps)) or True
            ),
...
def _configure_video_run(core, *, map_faces):
    ...
    core.modules.globals.explicit_content_screen = False
    core.modules.globals.disclosure_mode = "metadata"
    ...
    def test_map_faces_disk_fallback_extracts_frames_before_processing(self):
        ...
        with _patched_core_import_stubs(calls, pipe_result=False) as core:
            ...
            with patch.object(core.os.path, "isfile", return_value=True):
                with patch.object(core, "_finalize_output"):
                    core.start()
