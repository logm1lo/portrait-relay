"""Frame processor loading and media workers.
    ...
    for frame_processor, state in modules.globals.fp_ui.items():
        if state and frame_processor not in current_processor_names:
            try:
                ...
                print(
                    f"Warning: Failed to load frame processor {frame_processor} requested by UI state."
                )
            except Exception as e:
                print(
                    f"Warning: Error loading frame processor {frame_processor} requested by UI state: {e}"
                )
            ...
            try:
                module_to_remove = next(
                    (
                        mod
                        for mod in FRAME_PROCESSORS_MODULES
                        if mod.__name__.endswith(f".{frame_processor}")
                    ),
                    None,
                )
                if module_to_remove:
...
def process_video(
    source_path: str, frame_paths: list[str], process_frames: Callable[[str, list[str], Any], None]
) -> None:
    progress_bar_format = (
...
def process_video_in_memory(source_path: str, target_path: str, fps: float) -> bool:
    ...
    if source_path and os.path.exists(source_path):
        ...
        if source_face is None:
            print(
                "[PORTRAIT-RELAY.CORE] Warning: No face detected in source image. "
    ...
    try:
        ...
        print(f"[PORTRAIT-RELAY.CORE] Failed to get video dimensions: {e}")
        return False
    ...
    for attempt, (enc, enc_opts) in enumerate(encoders_to_try):
        ...
        if attempt == 0 and is_hw_encoder:
            print(
                f"[PORTRAIT-RELAY.CORE] Hardware encoder '{enc}' failed, retrying with software encoder..."
            )
...
def _run_pipe_pipeline(
    ...
    reader = None
    writer = None
    writer_error = tempfile.TemporaryFile()
    ...
    try:
        ...
        print(f"[PORTRAIT-RELAY.CORE] Failed to start FFmpeg pipes: {e}")
    ...
    try:
        with tqdm(
            ...
            while True:
                ...
                if modules.globals.disclosure_mode == "visible+metadata":
                    ...
                    frame = draw_visible_label(frame)
        ...
        if writer.returncode != 0:
            writer_error.seek(0)
            stderr_out = writer_error.read().decode(errors="ignore").strip()
            if stderr_out:
                print(f"[PORTRAIT-RELAY.CORE] FFmpeg encoder error: {stderr_out}")
            return False

        return processed_count > 0 and os.path.isfile(temp_output_path)

    except BrokenPipeError:
        print("[PORTRAIT-RELAY.CORE] FFmpeg pipe broken (encoder may not be available).")
        return False
    except Exception as e:
        print(f"[PORTRAIT-RELAY.CORE] In-memory processing error: {e}")
        return False
    finally:
